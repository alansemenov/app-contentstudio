...
describe('bestUniqueMatch', () => {
    it('should match exact names regardless of case and punctuation', () => {
        expect(bestUniqueMatch(candidates, 'Blog Post!')).toEqual({ kind: 'match', value: 'blog' });
        expect(bestUniqueMatch(candidates, 'superhero')).toEqual({ kind: 'match', value: 'superhero' });
    });

    it('should prefer the exact match over containment matches', () => {
        expect(bestUniqueMatch(candidates, 'article')).toEqual({ kind: 'match', value: 'article' });
    });

    it('should match a unique prefix', () => {
        expect(bestUniqueMatch(candidates, 'blog')).toEqual({ kind: 'match', value: 'blog' });
        expect(bestUniqueMatch(candidates, 'super')).toEqual({ kind: 'match', value: 'superhero' });
    });

    it('should match a unique containment in either direction', () => {
        expect(bestUniqueMatch(candidates, 'news')).toEqual({ kind: 'match', value: 'news' });
        expect(bestUniqueMatch(candidates, 'the superhero site')).toEqual({ kind: 'match', value: 'superhero' });
    });

    it('should match when every spoken word appears in the label', () => {
        expect(bestUniqueMatch(candidates, 'post blog')).toEqual({ kind: 'match', value: 'blog' });
    });
