...
// Drops a leading run of words that is the ending of one of Juke's recent
// replies, in order. Without a pause between the reply and the answer, Chrome
...
export function stripEchoPrefix(transcript: string, spoken: Spoken): string {
    ...
    return transcriptWords.slice(longest).join(' ');
}
